import { InfluxDB, type FluxTableMetaData } from '@influxdata/influxdb-client';
...
export const fetchSensorData = async (range: string) => {
  const data: any[] = [];
  const query = `
    from(bucket: "${bucket}")
    |> range(start: ${range})
    ...
    |> keep(columns: ["_time", "temp", "hum", "lux", "noise_db", "co2"])
  `;
